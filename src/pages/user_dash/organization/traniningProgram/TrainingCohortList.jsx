...
const TrainingCohortList = () => {
  ...
  // console.log("De" , organizationCohorts);
  ...
  const filteredCohorts = organizationCohorts.filter(cohort => {
    // Handle new data structure with subscription wrapper
    ...
    const matchesSearch = 
      ...
      cohortData.course?.title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      cohort.course_title?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      subscription?.paid_by?.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      subscription?.paid_by?.email?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const paymentStatus = subscription?.status || cohort.payment_status;
    ...
    const matchesSubscriptionType = filterSubscriptionType === 'all' || subscriptionType === filterSubscriptionType;
    
    return matchesSearch && matchesFilter && matchesPaymentFilter && matchesSubscriptionType;
  });
    ...
    <div className="training-program-container">
      ...
      <div className="training-program-filters">
        ...
        <div className="filter-group">
          ...
          <select value={filterSubscriptionType} onChange={(e) => setFilterSubscriptionType(e.target.value)}>
      ...
      <div className="training-program-stats">
        ...
        <div className="stat-card">
          ...
          <p className="stat-value">
            {organizationCohorts.filter(c => c.status === 'paid').length}
          </p>
        </div>
        <div className="stat-card">
                    ...
                    <tr key={subscription.id} style={{ borderBottom: '1px solid #e5e7eb' }}>
                      ...
                      <td style={{ padding: '1rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                          ...
                          {subscription.status && (
                            <span className={`status-badge ${subscription.status === 'paid' ? 'active' : 'inactive'}`}>
                              {subscription.status}
                            </span>
                          )}
                        </div>
                      ...
                      <td style={{ padding: '1rem', textAlign: 'center' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', alignItems: 'center' }}>
                          ...
                          {subscription.status !== 'paid' && (cohort.price > 0 || subscription.amount_paid > 0) && (
                            <button 
