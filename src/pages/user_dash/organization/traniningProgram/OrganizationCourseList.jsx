...
const OrganizationCourseList = () => {
  ...
  const { cohorts, loading: cohortsLoading } = useSelector((state) => state.cohorts);
    ...
    return (
      <div className="training-program-container">
        ...
        <div className="training-program-table-container">
          {[1, 2, 3].map(i => (
            <div key={i} className="cohort-card" style={{ marginBottom: '1rem' }}>
              <Skeleton height={100} />
            </div>
          ))}
        </div>
            ...
            return (
              <div key={course.id} className="course-card" style={{ 
                ...
                <div className="course-header" style={{ marginBottom: '1rem' }}>
                  ...
                  <p style={{ color: '#6b7280', marginTop: '0.5rem' }}>{course.description}</p>
                </div>

                <div className="cohorts-section">
                  <h3 style={{ fontSize: '1rem', fontWeight: '500', color: '#374151', marginBottom: '1rem' }}>Available Cohorts</h3>
                  {courseCohorts.length > 0 ? (
                    <div className="cohorts-grid" style={{ display: 'grid', gap: '1rem', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))' }}>
                      {courseCohorts.map(cohort => (
                        <div key={cohort.id} className="cohort-item" style={{ 
                          ...
                          <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '0.5rem' }}>
                              <h4 style={{ fontWeight: '600', color: '#111827' }}>{cohort.name}</h4>
                              <span className={`status-badge ${cohort.status || 'active'}`} style={{ fontSize: '0.75rem' }}>
                                {cohort.status || 'Active'}
                              </span>
                            </div>
                            <div style={{ fontSize: '0.875rem', color: '#6b7280', marginBottom: '1rem' }}>
                              <p>Start: {new Date(cohort.start_date).toLocaleDateString()}</p>
                              <p>End: {new Date(cohort.end_date).toLocaleDateString()}</p>
                              <p>Price: ₦{parseFloat(cohort.price).toLocaleString()}</p>
                            </div>
                          </div>
                          <button 
                    ...
                    <p style={{ color: '#9ca3af', fontStyle: 'italic' }}>No active cohorts available for this course.</p>
                  )}
                </div>
              </div>
            );
